...
| `/bf:poc <idea>` | Workflow | Interview + brainstorm → coherence-checked brief → fast build under dev conventions → smoke run → code/brief sync check; no git, every decision in a `BRIEF.md` ADR log | You want to prove an idea fast, and keep the decisions in case it becomes the real thing |
