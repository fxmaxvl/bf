...
To find a sub-skill's SKILL.md, use: `Glob("~/.claude/skills/bfeature-full/**/SKILL.md")` — then match by the path column in the routing table below. If the Glob returns no results, try `Glob("skills/bfeature-full/**/SKILL.md")` (some setups use a local `skills/` directory).
...
|-----------|---------------|------------|-------|-----------|
| brainstorm (gather) | `bfeature-full/brainstorm/SKILL.md` | Inline | — | Interactive Q&A — must stay in main conversation |
| brainstorm/generate | `bfeature-full/brainstorm/generate/SKILL.md` | Agent | opus | Spec synthesis from Q&A — reasoning-heavy, no interaction needed |
| refine | `bfeature-full/refine/SKILL.md` | Inline | — | Interactive Q&A — must stay in main conversation (quick mode only) |
| review-design | `bfeature-full/review-design/SKILL.md` | Agent | opus | Architectural analysis — produces report, no user interaction |
| review-design/fix | `bfeature-full/review-design/fix/SKILL.md` | Agent | sonnet | Applies spec fixes — execution task |
| plan | `bfeature-full/plan/SKILL.md` | Agent | opus | Deep reasoning for TDD blueprints |
| do-todo | `bfeature-full/do-todo/SKILL.md` | Agent | sonnet | Fast, execution-focused coding |
| verify | `bfeature-full/verify/SKILL.md` | Agent | sonnet | Quality gates — runs tests (monorepo-aware) and lint with auto-fix |
| review-impl | `bfeature-full/review-impl/SKILL.md` | Agent | opus | Implementation analysis — produces report, no user interaction |
| review-impl/fix | `bfeature-full/review-impl/fix/SKILL.md` | Agent | sonnet | Applies code fixes — execution task |
| collect-todos (Phase 7, optional) | `bfeature-full/collect-todos/SKILL.md` | Agent | sonnet | Mechanical scanning task — skipped if user declines |
...
5. If it exists: run `bash ~/.claude/skills/bfeature-full/scripts/state-ops.sh` to load state and resume:
...
2. **Detect Jira ticket:** Check if `$ARGUMENTS` contains a Jira ticket URL (e.g., `https://<domain>.atlassian.net/browse/PROJ-123` or similar). If it does:
   ...
   - Invoke the `bfeature-jira` skill to verify Jira MCP tools are available. If not available, stop.
   ...
   - Invoke the `bfeature-jira` skill: `transition-to(ticket_key, "In Progress")`
...
```
bash ~/.claude/skills/bfeature-full/scripts/state-ops.sh --init \
  --slug "<slug>" \
  --idea "<idea>" \
...
3. Read `bfeature-full/brainstorm/SKILL.md` and follow its instructions **inline** (in the current conversation) with the synthesized description
...
4. Read `bfeature-full/brainstorm/generate/SKILL.md` and pass its contents as an Agent prompt (model: opus) to produce the spec from the Q&A

### If `jira.enabled` is `false`:
1. Read `bfeature-full/brainstorm/SKILL.md` and follow its instructions **inline** (in the current conversation) with the idea from state
   - Runs in the main conversation — user interaction is fully available
   - Gather saves Q&A to `.claude/.bfeature-temp/<build_timestamp>-<slug>-qa.md`
2. Read `bfeature-full/brainstorm/generate/SKILL.md` and pass its contents as an Agent prompt (model: opus) to produce the spec from the Q&A
...
When the file at `paths.spec` is detected:
   ```
   bash ~/.claude/skills/bfeature-full/scripts/state-ops.sh \
...
1. Read `bfeature-full/refine/SKILL.md` and follow its instructions **inline** (in the current conversation) with the idea from state
   - Runs in the main conversation — user interaction is fully available
   - Saves Q&A to `.claude/.bfeature-temp/<build_timestamp>-<slug>-qa.md`
2. When the file at `paths.qa` is detected:
   ```
   bash ~/.claude/skills/bfeature-full/scripts/state-ops.sh phase=plan phase_status=in_progress
   ```
...
1. Read `bfeature-full/review-design/SKILL.md` and pass its contents as an Agent prompt (model: opus)
...
4. If `STATUS: CONCERN`:
   ...
   - If yes: read `bfeature-full/review-design/fix/SKILL.md` and pass its contents as an Agent prompt (model: sonnet), then go back to step 1
...
1. Read `bfeature-full/plan/SKILL.md` and pass its contents as an Agent prompt (model: opus) — it reads the appropriate source based on `mode` and produces `.claude/.bfeature-temp/<build_timestamp>-<slug>-plan.md` + `.claude/.bfeature-temp/<build_timestamp>-<slug>-todo.md`
2. When both files are detected:
   ...
   - If yes: `bash ~/.claude/skills/bfeature-full/scripts/state-ops.sh phase_status=in_progress` — proceed to Phase 4
...
1. Read `bfeature-full/do-todo/SKILL.md` and pass its contents as an Agent prompt (model: sonnet) — it loops internally until all items are checked
2. When it completes:
   ```
   bash ~/.claude/skills/bfeature-full/scripts/state-ops.sh phase=verify phase_status=in_progress
   ```
...
1. Read `bfeature-full/verify/SKILL.md` and pass its contents as an Agent prompt (model: sonnet)
...
2. When tests and lint are green:
   ```
   bash ~/.claude/skills/bfeature-full/scripts/state-ops.sh phase=review-impl phase_status=in_progress
   ```
...
1. Read `bfeature-full/review-impl/SKILL.md` and pass its contents as an Agent prompt (model: opus)
2. Read `paths.impl_report`
3. If `STATUS: PASS`: proceed to step 5
4. If `STATUS: CONCERN`:
   - Show the concerns to the user
   - Ask: "Should I fix these concerns?"
   - If yes: read `bfeature-full/review-impl/fix/SKILL.md` and pass its contents as an Agent prompt (model: sonnet), then go back to step 1
   - If no (user accepts as-is): proceed to step 5
...
5. ```
   bash ~/.claude/skills/bfeature-full/scripts/state-ops.sh phase=finalize phase_status=in_progress
   ```
...
1. **Silent quality gate:** Before touching git, read `bfeature-full/verify/SKILL.md` and pass its contents as an Agent prompt (model: sonnet) one final time.
...
2. **Pre-finalization gate:** Run:
   ```
   bash ~/.claude/skills/bfeature-full/scripts/state-ops.sh phase_status=awaiting_approval
   ```
   ...
   Then: `bash ~/.claude/skills/bfeature-full/scripts/state-ops.sh phase_status=in_progress collect_todos=<true|false>` — continue.
...
8. ```
   bash ~/.claude/skills/bfeature-full/scripts/state-ops.sh phase=collect-todos phase_status=in_progress
   ```
...
1. Read `bfeature-full/collect-todos/SKILL.md` and pass its contents as an Agent prompt (model: sonnet)
...
3. When complete:
   ```
   bash ~/.claude/skills/bfeature-full/scripts/state-ops.sh artifacts.backlog="<build_timestamp>-<slug>-backlog.md"
   # or if no items found:
   bash ~/.claude/skills/bfeature-full/scripts/state-ops.sh artifacts.backlog=null
   ```
...
```
bash ~/.claude/skills/bfeature-full/scripts/state-ops.sh phase=<phase> phase_status=<status>
```

The script auto-sets `updated_at`. Use dot notation for nested fields:

```
bash ~/.claude/skills/bfeature-full/scripts/state-ops.sh artifacts.plan=20260409T14-dark-mode-plan.md
bash ~/.claude/skills/bfeature-full/scripts/state-ops.sh collect_todos=true
bash ~/.claude/skills/bfeature-full/scripts/state-ops.sh phase=execute phase_status=awaiting_approval
```
